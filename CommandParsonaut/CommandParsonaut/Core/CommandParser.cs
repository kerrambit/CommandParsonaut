	...
	public class CommandParser
	{
		...
		public bool GetUnprocessedInput(out string input)
		{
			...
			while (true)
			{
				...
				{
					// TODO: Handle special commands, such as CTRL+L. TODO: this will not be hardcoded, basically there will be ADirective, from which
					...
					{
						if (builder.Length > 0 && _reader.GetCursorLeftPosition() - TerminalPromt.Length - 1 >= 0)
						{
					...
					{
						int cursorLeftPosition = _reader.GetCursorLeftPosition();
						if (cursorLeftPosition + 1 >= Console.WindowWidth)
						{
							continue;
						}
						...
						if (key.KeyChar == '\t')
						{
							builder = builder.Insert(builderOffset, "    ");
							builderOffset += 3;
						}
						else
						{
